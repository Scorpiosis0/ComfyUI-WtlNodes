...
def apply_rotation(image, angle, interp_method, fit_mode):
    ...
    for b in range(batch_size):
        ...
        if fit_mode == "crop":
            ...
            rotated = cv2.warpAffine(img_uint8, rotation_matrix, (w, h), 
...
class ImageRotationC:
    ...
    def rotate(self, image, rotate, interpolation, fit_mode, apply_type, unique_id=None):
        ...
        if unique_id and not apply_type == "auto_apply":
            ...
            if apply_type == "apply_all":
                while True:
                    ...
                    cur_image = apply_rotation(image, cur_rotate, cur_method, cur_fit)
                ...
                result = apply_rotation(image, rotate, interp_method, fit_mode)
                ...
                for i in range(batch_size):
                    ...
                    while True:
                        cur_rotate, cur_interp, cur_fit = _get_params(uid, rotate, interpolation, fit_mode)
                        cur_method = self.INTERPOLATION_METHODS[cur_interp]
                        cur_image = apply_rotation(single_image, cur_rotate, cur_method, cur_fit)
                        _send_ram_preview(cur_image, uid)
                    ...
                    if final_rotate is not None:
                        processed = apply_rotation(single_image, final_rotate, final_method, final_fit)
...
NODE_DISPLAY_NAME_MAPPINGS = {"ImageRotation": "Image Rotate"}